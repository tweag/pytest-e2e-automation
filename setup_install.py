...
def install_project_selenium_drivers():
    print_green(_ := "Installing Drivers")
    print_green("-" * len(_))
    ...
    print_green(f"Successfully Downloaded Drivers at location {BINARIES_DIR}")
    print("\n")


def run_project_install_tests():
    import pytest
    ...
    print_green("-" * len(_))
    os.environ['PROJECT_INSTALLATION'] = 'True'
    ...
    if test == 0:
        ...
        print_red(
            "Please review the setup logs and fix the errors before proceeding with the project"
        )
...
class PlatformDetails:
    ...
    def check_for_python_v3_9(self) -> None:
        if not self.py_ver.startswith("3.9"):
            sys.exit(
                BColors.FAIL
                + " Python Version is incorrect. Please use Python 3.9.x"
                + BColors.ENDC
            )
...
class Setup:
    ...
    def __init__(self):
        self.venv_dir = Path(os.environ.get("HOME"), ".qa-venv")
        self.req_file = Path().cwd() / self.REQUIREMENTS_FILE_NAME
        self.custom_req_files = [Path().cwd() / "backend" / self.REQUIREMENTS_FILE_NAME, Path().cwd() / "e2e" / self.REQUIREMENTS_FILE_NAME]

    @staticmethod
    def _check_project_prerequisites():
    ...
    def _install_reqs(self):
        def __print_failed_libs(failed_libs: list):
            ...
            for lib in failed_libs:
                print_red("\t", "|", lib)
            print_red("Aborting Setup Process due to failed dependencies.")
            sys.exit(1)
        ...
        for custom_req_file in self.custom_req_files:
            if custom_req_file.is_file():
                install_custom_failure_status = InstallRequirements(custom_req_file).install_c_reqs()
                if install_custom_failure_status:
                    __print_failed_libs(install_custom_failure_status)

    def global_setup(self):
        self._check_project_prerequisites()
        self._create_venv()

    def venv_setup(self):
        self._install_reqs()
        install_project_selenium_drivers()
        run_project_install_tests()
